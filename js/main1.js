...
function getBgColor(el) {
    ...
    let _bg;
    let _gr;
    let gradientStr, gradientParts;
    let gradientsHexArray = [];
    ...
    if (_gr !== "none") {
        gradientStr = _gr.substring(_gr.indexOf('(') + 1, _gr.lastIndexOf(')'));
        gradientParts = gradientStr.split(/,(?![^(]*\))(?![^"']*["'](?:[^"']*["'][^"']*["'])*[^"']*$)/);
        gradientParts.forEach(grad => {
            let rgbRegex = new RegExp(/rgb\((\d{1,3}), (\d{1,3}), (\d{1,3})\)/);
            let percentRegex = new RegExp(/\d{1,3}%/);
            let color = grad.match(rgbRegex)[0];
            let percent = grad.match(percentRegex)[0];
            color = tinycolor(color).toHexString();
            gradientsHexArray.push(
                [
                    color,
                    percent
                ]);
    ...
    gradientsHexArray.forEach((g, index) => {
        let coma = index === gradientsHexArray.length-1 ? "" : ",";
        gradientString += `${g[0]} ${g[1]}${coma}`;
    })
    gradientString += `)`;

    if (_bg === "rgba(0, 0, 0, 0)" && _gr === "none") {
        let _parent = _el.parentElement;
        return getBgColor(_parent);
    } else {
        return {
            bg: _gr !== "none" ? tinycolor(gradientsHexArray[0][0]).toHexString() : tinycolor(_bg).toHexString(),
            g: _gr !== "none" ? gradientString : tinycolor(_bg).toHexString(),
        }
    }
}
...
function getBorderColor(el) {
    ...
    let _bg;
    let _style = window.getComputedStyle(_el);
    _bg = _style.getPropertyValue('border-color');
    return _bg === "rgba(0, 0, 0, 0)" ? false : tinycolor(_bg).toHexString();
}
...
function getBg(el, essence, shift) {
    ...
    let isLight;
    config[data.nameG] = getBgColor(_el).g;
...
function createMap() {
    ...
    let dominantEl = document.querySelector(".tg_search_bg");
    ...
    let popupEl = document.querySelector(".tg_search_bg");
    ...
    let tabEl = document.querySelector(".dg_game_tabs .tab_selector");
    let tabActiveEl = document.querySelector(".dg_game_tabs .tab_selector.tab_selector_active");
    let menu_1El = document.querySelector(".tg__left_menu_item.tg--flex");
    ...
    let map = {
        ...
        ...getBg(inputEl, "input"),
        ...getBg(dominantEl, "inputSecondary", true),
        ...getBg(headerEl, "header"),
        ...getBg(subHeaderEl, "subHeader"),
        ...getBg(eventEl, "event"),
        ...getBg(buttonEl, "button"),
        ...getBg(dominantEl, "buttonSecondary", true),
        ...getBg(popupEl, "popup"),
        ...getBg(oddEl, "odd"),
        ...getBg(oddActiveEl, "oddActive"),
        ...getBg(showMoreEl, "showMore"),
        ...getBg(tabEl, "tab"),
        ...getBg(tabActiveEl, "tabActive"),
        ...getBg(tabEl, "tabSecondaryActive", true),
        ...getBg(dominantEl, "filter", true),
        ...
        ...getAccent(dominantAccentEl, "input", true),
        ...getAccent(dominantAccentEl, "inputSecondary", true),
        ...getAccent(dominantAccentEl, "header", true),
        ...getAccent(dominantAccentEl, "subHeader", true),
...
function createPartnerMap(config) {
    ...
    let _str = {
        ...
        inputBorder: _config.inputBgHover,
        inputBorder: _config.inputBorder ? _config.inputBorder : _config.inputBg,
        ...
        buttonBorder: _config.buttonBorder ? _config.buttonBorder : _config.buttonBg,
        ...
        /*popup*/
        popupBg: _config.popupBg,
        popupBg2: _config.popupBg2,
        popupBg3: _config.popupBg3,
        popupBgHover: _config.popupBgHover,
        popupBg2Hover: _config.popupBg2Hover,
        popupBg3Hover: _config.popupBg3Hover,
        popupTxt: _config.popupTxt,
        popupTxt2: _config.popupTxt2,
        popupTxt3: _config.popupTxt3,
        popupAccent: _config.accentBg,
        popupAccentTxt: _config.accentTxt,
        popupBorder: _config.popupBgHover,
...
let partnersIdArray = [1, 100, 105, 107, 109, 11, 110, 113, 114, 117, 118, 119, 120, 121, 122, 125, 126, 127, 128, 130, 131, 132, 134, 136, 137, 139, 14, 143, 144, 145, 146, 147, 148, 157, 163, 164, 165, 166, 167, 169, 170, 173, 179, 181, 182, 183, 185, 188, 189, 190, 191, 193, 197, 198, 200, 201, 205, 206, 209, 210, 211, 212, 213, 215, 217, 221, 222, 225, 226, 227, 228, 23, 230, 231, 233, 234, 235, 236, 237, 238, 24, 240, 243, 245, 247, 248, 249, 250, 252, 253, 254, 255, 256, 257, 258, 259, 260, 261, 262, 264, 265, 266, 267, 268, 269, 271, 272, 273, 277, 279, 280, 281, 282, 283, 284, 285, 286, 29, 290, 291, 294, 295, 297, 299, 300, 3000004, 3000012, 3000013, 3000023, 301, 302, 303, 304, 305, 307, 308, 309, 310, 311, 312, 313, 314, 316, 317, 318, 319, 321, 322, 323, 324, 325, 326, 327, 328, 329, 331, 332, 333, 334, 335, 336, 337, 340, 341, 342, 343, 344, 346, 350, 355, 357, 358, 359, 360, 362, 368, 369, 384, 385, 387, 388, 389, 390, 391, 392, 393, 394, 395, 396, 397, 398, 399, 40, 400, 401, 402, 403, 404, 406, 407, 409, 41, 410, 412, 413, 414, 415, 416, 418, 419, 421, 422, 424, 425, 427, 428, 429, 43, 430, 433, 434, 435, 436, 439, 441, 444, 446, 449, 45, 451, 452, 455, 456, 457, 458, 46, 461, 464, 53, 54, 62, 63, 64, 65, 69, 71, 72, 76, 77, 79, 80, 81, 83, 89, 9, 90, 94, 95, 97, 98, 99]
let partnerIndex = -1;
...
function attachPartnerStyle(id, callback) {
...
window.addEventListener('keypress', (e) => {
    if (e.code === "KeyN") {
        partnerIndex++
        attachPartnerStyle(partnersIdArray[partnerIndex], () => createTheme(partnersIdArray[partnerIndex]));
    }
})


// let partnerStyle = document.getElementById('partnerStyleCssLink');
// let href = `/Partners/109/Styles/web.css`;
// partnerStyle.setAttribute('href', href);